...
export class HaravanService {
  ...
  async loginApp(orgid: string): Promise<string> {
    try {
      if (orgid === undefined || orgid === null || orgid.trim() === '' || orgid === "null") {
        ...
        const checkOrgidExists = await this.redisService.has(`haravan:faq:app_subscriptions:${orgid}`);
        if (!checkOrgidExists) {
          return await this.buildUrlInstall();
        }
      }
  ...
  async installApp(code: string, res): Promise<string> {
    ...
    try {
      ...
      const existingApp = await this.redisService.get(`haravan:faq:app_install:${orgid}`);
      ...
      await this.redisService.set(`haravan:faq:app_install:${orgid}`, tokenData, 30 * 24 * 60 * 60); // 30 days
  ...
  async refreshToken(orgid, old_refresh_token) {
    ...
    try {
      ...
      const existsApp = await this.redisService.get(`haravan:faq:app_install:${orgid}`);
      if (!existsApp) {
        await this.redisService.set(`haravan:faq:app_install:${orgid}`, tokenData, 30 * 24 * 60 * 60); // 30 days
      } else {
        existsApp.access_token = access_token;
        existsApp.refresh_token = refresh_token;
        await this.redisService.set(`haravan:faq:app_install:${orgid}`, existsApp, 30 * 24 * 60 * 60); // 30 days
      }
      return access_token;
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      switch (hrvTopic) {
        case 'app_subscriptions/update':
          ...
          const existsApp = await this.redisService.get(`app_install:${orgid}`);
          if (existsApp) {
            existsApp.status = status; 
            existsApp.expires_at = +new Date(expired_at);
            await this.redisService.set(`app_install:${orgid}`, existsApp, 30 * 24 * 60 * 60); // 30 days
          }
          if (status === 'active') {
            console.log(`Updating subscription for orgid: ${orgid}`);
            await this.redisService.set(`app_subscriptions:${orgid}`, body, ttlSeconds);
          } else {
            console.log(`Deleting subscription for orgid: ${orgid}`);
            await this.redisService.del(`app_subscriptions:${orgid}`);
          }
          break;
        default:
